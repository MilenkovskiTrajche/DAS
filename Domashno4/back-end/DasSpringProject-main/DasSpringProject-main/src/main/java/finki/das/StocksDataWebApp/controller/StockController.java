...
public class StockController {
    ...
    public ResponseEntity<?> getDataFromScraper() {
        String scriptPath = "/app/pythonScripts/webscrapper.py";

        // Run the webscraper first
        System.out.println("Webscraper started");
        boolean webscraperFinished = ScraperScriptRunner.startScript(scriptPath);

        if (!webscraperFinished) {
            return ResponseEntity.status(500).body("Webscraper script failed or timed out.");
        }

//        // Run other scripts sequentially after the webscraper is done
//        try {
//            System.out.println("Oscillator/Moving Average scripts started");
//            runIndicatorScripts();
//        } catch (IOException e) {
//            System.out.println("Indicator scripts failed: " + e.getMessage());
//            return ResponseEntity.status(500).body("Indicator scripts failed: " + e.getMessage());
//        }

        // Trigger scraping via REST call
        try {
            String scrapeUrl = "http://python-service:4000/";
            ...
            return ResponseEntity.status(500).body("Error triggering scraping: " + e.getMessage());
        }
    }

//    private void runIndicatorScripts() throws IOException {
//        String[] paths = {
//                "/pythonScripts/weeklyOscToDB.py",
//                "/pythonScripts/weeklyMovAvgToDB.py",
//                "/pythonScripts/monthlyOscillatorsToDB.py",
//                "/pythonScripts/monthlyMovAvgToDB.py"
//        };
//
//        for (String script : paths) {
//            boolean success = ScraperScriptRunner.startScript(script);
//            if (!success) {
//                throw new IOException("Script failed or timed out: " + script);
//            }
//        }
//    }
    ...
    public List<String> getSymbols(){
        String folder_path="/app/pythonScripts/Fetched_Files2";
