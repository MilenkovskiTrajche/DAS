...
DATABASE_PASSWORD = "useruser123"
TABLE_NAME = "moving_average"

input_directory = 'C:\\Users\\Com\\PycharmProjects\\dasDomashno\\Fetched_Files2'
...
def process_file(file_path, db_connection):
    try:
        ...
        # EMA CALCULATION
        df_weekly['EMA_5W'] = df_weekly['PriceOfLastTransaction'].ewm(span=5, adjust=False).mean()

        df_weekly['Signal_EMA'] = 'HOLD'
        df_weekly['EMA_5W'].fillna(0, inplace=True)

        for i in range(5, len(df_weekly)):
            if df_weekly['PriceOfLastTransaction'].iloc[i] > df_weekly['EMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_EMA'] = 'BUY'
            elif df_weekly['PriceOfLastTransaction'].iloc[i] < df_weekly['EMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_EMA'] = 'SELL'

        # SMA CALCULATION
        df_weekly['SMA_5W'] = df_weekly['PriceOfLastTransaction'].rolling(window=5).mean()
        df_weekly['SMA_5W'].fillna(0, inplace=True)

        df_weekly['Signal_SMA'] = 'HOLD'
        for i in range(5, len(df_weekly)):
            if df_weekly['PriceOfLastTransaction'].iloc[i] > df_weekly['SMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_SMA'] = 'BUY'
            elif df_weekly['PriceOfLastTransaction'].iloc[i] < df_weekly['SMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_SMA'] = 'SELL'

        # WMA CALCULATION
        weights = np.arange(1, 6)
        df_weekly['WMA_5W'] = df_weekly['PriceOfLastTransaction'].rolling(window=5).apply(
            lambda x: np.dot(x, weights[::-1]) / weights.sum())
        df_weekly['WMA_5W'].fillna(0, inplace=True)

        df_weekly['Signal_WMA'] = 'HOLD'
        for i in range(5, len(df_weekly)):
            if df_weekly['PriceOfLastTransaction'].iloc[i] > df_weekly['WMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_WMA'] = 'BUY'
            elif df_weekly['PriceOfLastTransaction'].iloc[i] < df_weekly['WMA_5W'].iloc[i]:
        ...
        df_weekly['TEMA_5W'] = 3 * ema1 - 3 * ema2 + ema3
        df_weekly['TEMA_5W'].fillna(0, inplace=True)

        df_weekly['Signal_TEMA'] = 'HOLD'
        for i in range(5, len(df_weekly)):
            if df_weekly['PriceOfLastTransaction'].iloc[i] > df_weekly['TEMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_TEMA'] = 'BUY'
            elif df_weekly['PriceOfLastTransaction'].iloc[i] < df_weekly['TEMA_5W'].iloc[i]:
        ...
        df_weekly['HMA_5W'] = hma
        df_weekly['HMA_5W'].fillna(0, inplace=True)

        df_weekly['Signal_HMA'] = 'HOLD'
        for i in range(5, len(df_weekly)):
            if df_weekly['PriceOfLastTransaction'].iloc[i] > df_weekly['HMA_5W'].iloc[i]:
                df_weekly.loc[df_weekly.index[i], 'Signal_HMA'] = 'BUY'
            elif df_weekly['PriceOfLastTransaction'].iloc[i] < df_weekly['HMA_5W'].iloc[i]:
